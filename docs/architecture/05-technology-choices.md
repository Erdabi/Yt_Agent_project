...
|---|---|---|---|
| LLM (ideation, scripting, quality control) | Anthropic Claude API | OpenAI GPT | Used for scoring, script generation, and the Quality Control Agent's reviewer checks. The Quality Control Agent's reviewers (`services/agent_qa/app/reviewers/`) are the one consumer that goes through a real, swappable `libs.providers.get_provider("llm")` capability (`libs/providers/llm/` — `LLMProvider.generate_tool_call`, forced tool-use, optional vision images) rather than a direct SDK call; `config/providers.yaml` defaults `llm.active` to `anthropic` since it's already a hard requirement elsewhere in this system. The Research/Script Agents and the Manager's reasoning engine still call `anthropic.Anthropic` directly (they predate this capability) — migrating them onto the same registry entry is a documented, low-risk future cleanup, not yet done. |
