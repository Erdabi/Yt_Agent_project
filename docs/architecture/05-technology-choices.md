...
| Video rendering/composition | FFmpeg (+ a thin Python compositing layer, e.g. `ffmpeg-python`/MoviePy) | — | Chosen over Remotion to avoid adding a Node.js runtime dependency alongside the Python stack; Remotion remains a documented alternative if programmatic, React-authored templates are wanted later. Not installed in the Video Agent's image yet — its Rendering module builds a real render plan but the actual compositor invocation is a documented `NotImplementedError` until it is. |
